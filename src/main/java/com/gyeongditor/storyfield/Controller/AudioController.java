...
public class AudioController {
    ...
    public ApiResponseDTO<String> uploadAudio(
        ...
        return audioService.uploadAudio(file, accessToken);
    }
    ...
    @ApiResponses({
                            ...
                            examples = @ExampleObject(value = """
            {
              "status": 200,
              "code": "AUDIO_200_003",
              "message": "오디오 파일 URL 조회 성공",
              "data": "https://s3-....amazonaws.com/bucket/audio/xxx.wav"
            }"""))),
            @ApiResponse(responseCode = "404", description = "오디오 파일을 찾을 수 없음",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(value = """
            {
              "status": 404,
                            ...
                            examples = @ExampleObject(value = """
            {
              "status": 500,
              "code": "AUDIO_500_003",
              "message": "오디오 파일 URL 조회 실패",
              "data": null
            }""")))
    })
    @GetMapping("/{fileName}")
    public ApiResponseDTO<String> getAudioUrl(
            @Parameter(description = "S3에 저장된 오디오 파일명 (서버에서 'audio/' prefix 자동 보정)", required = true)
            @PathVariable String fileName,
            @Parameter(description = "Bearer AccessToken", required = true)
            @RequestHeader("Authorization") String authorizationHeader
    ) {
        String accessToken = authorizationHeader.replace("Bearer ", "");
        return audioService.getAudioUrl(fileName, accessToken);
    }

    @Operation(
            summary = "다운로드용 Presigned URL 발급",
            description = "private 버킷에서 오디오 파일을 GET으로 내려받기 위한 Presigned URL을 발급합니다. 존재하지 않는 키면 404를 반환합니다."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "다운로드용 Presigned URL 생성 성공",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(value = """
            {
              "status": 200,
    ...
    public ApiResponseDTO<String> getDownloadPresignedUrl(
            @Parameter(description = "S3 오디오 키 또는 파일명(서버에서 'audio/' prefix 자동 보정)", required = true)
            @RequestParam String keyOrFileName,
            @Parameter(description = "Bearer AccessToken", required = true)
            @RequestHeader("Authorization") String authorizationHeader
    ) {
        String accessToken = authorizationHeader.replace("Bearer ", "");
        return audioService.generateDownloadPresignedUrl(keyOrFileName, accessToken);
    }

    @Operation(summary = "오디오 삭제", description = "S3에 업로드된 오디오 파일명을 기반으로 오디오 파일을 삭제합니다.")
    @ApiResponses({
    ...
    public ApiResponseDTO<Void> deleteAudio(
        ...
        String accessToken = authorizationHeader.replace("Bearer ", "");
        return audioService.deleteAudio(fileName, accessToken);
    }
}
